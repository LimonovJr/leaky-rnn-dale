...
def rollout_one_trial(env):
    ...
    NeuroGym builds the full observation array on reset(), so we just grab it.

    Returns x [T, obs_dim] float32, gt [T] int64.
    """
    env.reset()
    return env.ob.astype(np.float32).copy(), env.gt.astype(np.int64).copy()
...
def make_train_batch(env_fn, batch_size=64, dt=20, response_weight=1.0,
    ...
    Returns x [B, T, D], y [B, T], mask [B, T], lengths [B].
    """
    xs, ys, masks = [], [], []
