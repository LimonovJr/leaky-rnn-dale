...
def rollout_one_trial(env, xy_mask_from="cue"):
    """
    Read pre-generated trial from env.ob / env.gt without stepping.

    xy_mask_from: "cue" (default) -> xy_mask is 1 from cue onset to end;
                  "target"        -> xy_mask is 1 from target onset to end
                                     (spatial loss active only during/after target).
    ...
    Returns:
        ...
        xy_mask  [T]          float32  -- 1.0 from xy_mask_from onset to end, else 0.0
    """
    env.reset()
    T = len(env.gt)
    ...
    # aux loss active from cue onset (default) or target onset (xy_mask_from="target")
    mask_key = "target" if xy_mask_from == "target" else "cue"
    mask_start = int(env.start_ind[mask_key])
    xy_mask = np.zeros(T, dtype=np.float32)
    xy_mask[mask_start:] = 1.0
...
def make_train_batch(env_fn, batch_size=64, dt=20, response_weight=1.0,
    ...
    for _ in range(batch_size):
        env = env_fn()
        x_seq, gt_seq, fa_seq, xy_seq, xy_mask_seq = rollout_one_trial(env, xy_mask_from=xy_mask_from)
