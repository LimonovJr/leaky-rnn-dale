...
def fit_jpca(X_cond, n_jpcs=1, pca_dims=6):
    """
    Fit jPCA on condition-averaged trajectories (Churchland et al., 2012).

    Steps: mean-subtract -> PCA pre-reduce -> fit dX ≈ X @ M.T -> extract
    skew-symmetric part M_skew -> eigendecompose -> orthonormalize jPC pairs.
...
def jpca_permutation_test_paper(X_cond, n_perms=100, pca_dims=6, n_jpcs=1, seed=0):
    """
    Amengual et al. permutation test: shuffle conditions per neuron (preserves
...
def plot_spatial_map(model, h_mean, title="Spatial map"):
    """Plot mean activity [n_tot] on the 2D sheet (requires model.e_coords, model.i_coords)."""
    import numpy as np
    import matplotlib.pyplot as plt
